...
uses
  ...
  pausensteuerung, BackTrackDemo_Sprache;
...
const
     solitaerFileName = 'SolitaerSpielsicherung.txt';
     qSize = 7;
     qSizeQuadrat = qSize*qSize;
     qMitte = (qSize+1) div 2;
...
type
  ...
  TLinBrett    = array[1..maxFeldAnzahl] of TStatus;
  TEinzelzug   = record vx,vy,nx,ny:integer end;
  TZugfolge    = array[1..maxFeldAnzahl] of TEinzelzug;
  TVersuchsBrett = array[-1..qSize+2,-1..qSize+2] of TStatus;
  TBelegungBrett = array[1..qSize,1..qSize] of TStatus;
  ...
  private
    ...
    brettVorgabe, brettManuellPlay : TVersuchsBrett;
    brett            : TBelegungBrett;
    ...
    zug, mZug: TZugfolge; // array[1..maxFeldAnzahl] of TEinzelzug;
    LinZug   : TLinZugFolge;
    ...
    mZugNr, zugNr, zugNrOnESC,
    lastx, lasty, lastFeldx,
    lastFeldy, sx,sy, sprung,lastBrettType  : integer;
    testCounter, testMrd,
    startTime, endTime        : longint;
    restAnzahlPins,anzLsg     : integer;
    autoPlay, pinAusgewaehlt,
    speichern, laden, findAll : boolean;
    speicherListe             : TStringList;
    procedure BelegeBrett(vonx,vony,nachx,nachy:integer; aStatus:TStatus);
    procedure BelegeManuBrett(vonx,vony,nachx,nachy:integer; aStatus:TStatus);
    procedure CheckInteraction;
    function GetRichtungsChar(aktZ:TEinzelzug):char;
    procedure FuelleListBox(z:TZugFolge);
    procedure InitBrett(brettType:integer);
    procedure InitBrettFormat(brettForm:integer);
    procedure InitBrettZaehlung(art:integer);
    procedure InitStartVars;
    procedure InitTestZugArray(startMove:TSprungRtg);
    procedure InitZugVon;
    procedure LinBrett_To_Brett;
    ...
    procedure ZeigeZugFolge(aZug:TZugFolge;nr:byte);
...
procedure TFormSolitaer.BelegeBrett(vonx,vony,nachx,nachy:integer; aStatus:TStatus);
var mx,my:integer;
begin
  mx:=(vonx+nachx) div 2;
  my:=(vony+nachy) div 2;
  Brett[vonx,vony]:=aStatus;
  Brett[mx, my]:=aStatus;
  if aStatus=frei
    then begin
           Brett[nachx,nachy]:=belegt;
           restAnzahlPins:=restAnzahlPins-1;
           zugNr:=zugNr+1;
           with zug[zugNr] do begin vx:=vonx; vy:=vony; nx:=nachx;  ny:=nachy end;
           if zeigen then
             begin
               MoveStein(vonx,vony, nachx,nachy);
               LabelAnzahlTests.caption:=intToStr(testCounter);
             end;
         end
    else begin
           Brett[nachx,nachy]:=frei;
           restAnzahlPins:=restAnzahlPins+1;
           zugNr:=zugNr-1;
           if zeigen then
             begin
               ZeichneFeld(vonx,vony,belegt);
               ZeichneFeld(mx,my,belegt);
               ZeichneFeld(nachx,nachy,frei);
             end;
         end;
end;

procedure TFormSolitaer.BelegeManuBrett(vonx,vony,nachx,nachy:integer; aStatus:TStatus);
var mx, my  : integer;
    revStat : TStatus;
begin
  mx:=(vonx+nachx) div 2;
  ...
  if aStatus=frei
    then begin
           ...
           restAnzahlPins:=restAnzahlPins-1;
           mZugNr:=mZugNr+1;
           with mZug[mZugNr] do begin vx:=vonx; vy:=vony; nx:=nachx;  ny:=nachy end;
         end
    else begin
           revStat:=frei;
           restAnzahlPins:=restAnzahlPins+1;
  ...
  brettManuellPlay[vonx,vony]:=aStatus;
  brettManuellPlay[mx,my]:=aStatus;
  brettManuellPlay[nachx,nachy]:=revStat;
  ZeichneFeld(vonx,vony,aStatus);
  ZeichneFeld(mx,my,aStatus);
  ZeichneFeld(nachx,nachy,revStat);
  buttonZugZurueck.Visible:=mZugNr>0;
end;
...
begin
  if ButtonAbbruchUndSpeichern.tag=0
    then begin
           ...
           laden:=true;
           ButtonAbbruchUndSpeichern.Caption:=RSArr[rsAbbruchSpeichern];
...
begin
  if buttonManuell.Tag=0
    then begin
           InitBrett(1);
           InitTestZugArray(TSprungRtg(bf_StartRichtung));
           autoPlay:=false;
           mZugNr:=0;
           mZug:=Default(TZugFolge);
           buttonManuell.Tag:=1;
           buttonManuell.Caption:=RSArr[rsSpielBeenden];
           ...
           ButtonZugZurueck.Visible:=false;;
           buttonManuell.Tag:=0;
           buttonManuell.Caption:=RSArr[rsManuell];
           abbruch:=true;
           InitBrett(1);
           autoPlay:=true;
           ZeigeErgebnis(false);
...
begin
  if autoplay
    then begin
           if (bf_verfahren>2) then
             begin
               LinBrett_To_Brett;
               Umrechnen_LinZug_to_ZugXY(LinZug);
             end;
           ZeigeZugFolge(zug,zugNr)
         end
    else ZeigeZugFolge(mZug,mZugNr);
end;

procedure TFormSolitaer.ButtonZugZurueckClick(Sender: TObject);
begin
  with mZug[mZugNr] do BelegeManuBrett(vx, vy, nx, ny, belegt);
end;
...
begin
  ...
  case bf_verfahren of
      1 : begin     // BrutForce rekursiv
            ...
            if laden
              then begin
                     speicherListe:=TStringList.Create;
                     speicherListe.LoadFromFile(solitaerFileName);
              ...
              if speichern then
                begin
                  ...
                  while j<=zugNrOnESC do
                    begin
                      n:=brettXYToNr[ zug[j].vx, zug[j].vy ];
                      speicherListe.Add(IntTostr(n));
                      s:=GetRichtungsChar(zug[j]);
                      speicherListe.Add(s);
                      j:=j+1;
                    end;
                  speicherListe.SaveToFile(solitaerFileName);
                  if FileExistsUTF8(solitaerFileName) { *Converted from FileExists* }
                    then begin
                           ButtonAbbruchUndSpeichern.Caption:=RSArr[rsFortsetzen];
  ...
  if erfolg then
    if bf_verfahren > 2 then Umrechnen_LinZug_to_ZugXY(linZug);
  ZeigeErgebnis(erfolg);
  memo1.Lines.Add(RSArr[rsZeit]+intToStr(endTime-startTime)+RSArr[rsMs]);
end;
...
begin
  ...
  for k:=0 to maxGame-1 do
    ComboBoxBrettFormat.Items.Add(RSArr[k + rsKreuz]);
...
procedure TFormSolitaer.FuelleListBox(z:TZugFolge);
var k : integer;
    s : string;
begin
  ...
  for k:=1 to zugNr do
    begin
      s:= GetRichtungsChar(z[k]);
      with z[k] do s:='('+inttostr(vx)+'/'+ inttostr(vy)+') '+ s +'     ('+inttostr(nx)+'/'+ inttostr(ny)+')';
      listbox1.Items.Add(s);
    end;
end;

function TFormSolitaer.GetRichtungsChar(aktZ:TEinzelzug):char;
begin
  if aktZ.vx=aktZ.nx
    then if aktZ.ny>aktZ.vy
           then result:=nach_unten
           else result:=nach_oben
    else if aktZ.nx>aktZ.vx
           then result:=nach_rechts
           else result:=nach_links;
end;

procedure TFormSolitaer.InitBrett(brettType:integer);

  procedure InitStartPosition(out startPosX, startPosY : byte);
  begin
    startPosX := qMitte;
    startPosY := qMitte;
    endPosX := qMitte;
    ...
    case brettFormat of
      _kreuz, _Plus, _Kamin,
      _Acht, _Pyramide      :  startPosY:=qSize;
...
begin
  ...
  InitStartPosition(startX, startY);

  if brettType and 1 = 1 then
    begin
      brettManuellPlay:=brettVorgabe; // Move(brettVorgabe, brettManuellPlay, sizeOf(brettVorgabe));
      brettManuellPlay[startX,startY]:=frei;
      restAnzahlPins:=maxAnzahlPins-1;
    end;
  if brettType and 2 = 2 then
    begin
      for y:=1 to qSize do
        for x:=1 to qSize do
          brett[x,y]:=brettVorgabe[x,y];
      brett[startX,startY]:=frei;
      restAnzahlPins:=maxAnzahlPins-1;
   end;
  if brettType and 4 = 4 then
    begin
      ...
      linBrett[BrettXYToNr[startX,startY]]:=frei;
    end;
end;
...
procedure TFormSolitaer.InitTestZugArray(startMove:TSprungRtg);
var x, y, nx, ny, r, k, j, n : integer;
begin
  n := Ord(startMove)+1;
  j:=1;
  for k:=1 to maxAnzahlBrettPos do
    begin
      ...
      for r := n to n+3 do    //n=1: Reihenfolge: rechts, oben, links, unten
        begin                 //n=2: Reihenfolge: oben, links, unten, rechts
          case r of
              1,5 : begin nx:=x+2; ny:=y; end;  // rechts
              2,6 : begin nx:=x; ny:=y-2; end;  // oben
              3,7 : begin nx:=x-2; ny:=y; end;  // links
               else begin nx:=x; ny:=y+2; end;  // unten
          end;
          if brettVorgabe[nx,ny]<>ungueltig then
            begin
              testZug[j].von  := k;
              testZug[j].ueber:= brettXYtoNr[(x+nx) div 2,(y+ny) div 2];
...
procedure TFormSolitaer.LinBrett_To_Brett;
var k : integer;
begin
 for k:=1 to maxAnzahlBrettPos do
...
var z, p, k  : integer;
    ...
    pkt      : TPoint;
begin
  tempZug:=Default(TZugFolge);
  z:=Memo1.CaretPos.y;
  s := memo1.Lines[z];
  p  := z*2;  // CRLF Anzahl
  for k := 0 to z - 1 do p := p + Length(Memo1.Lines[k]);
  memo1.SelStart:=p;
    ...
    begin
      ...
      if ( pos(zStr[1],'123456789')>0 ) and
        ...
        then begin
               ...
               tempZug[z].ny:=pkt.y;
               case s[k+2] of
                   nach_unten :  tempZug[z].ny:=pkt.y+2;
                   nach_oben :   tempZug[z].ny:=pkt.y-2;
                   nach_rechts : tempZug[z].nx:=pkt.x+2;
                   nach_links :  tempZug[z].nx:=pkt.x-2;
                  end;
               k:=k+4;
            end
       else begin  memo1.SelLength:=0;  exit; end;
    end;
  if Length(s)=4*AnzahlZuegeBisLoesung then
    begin
      FuelleListBox(tempZug);
      listbox1.Visible := true;
      zugNr:= AnzahlZuegeBisLoesung;
    end;;
  ZeigeZugFolge(tempZug,z);
  memo1.SelLength:=0;
end;
...
var k,t,neux,neuy : integer;

   procedure Umsetzen;
   begin
     BelegeManuBrett(sx, sy, neux, neuy, frei);
     sprung:=0; sx:=0; sy:=0;
  ...
  if (not autoPlay) and (sprung<>0) and ( (lastFeldx<>0) and (lastFeldy<>0) )
    ...
    if (brettManuellPlay[lastFeldx,lastFeldy]=frei)
      then for k:=1 to 4 do
             begin
               case k of
                  1 : begin neux:=sx+2; neuy:=sy; t:=1 end;
                  2 : begin neux:=sx; neuy:=sy+2; t:=2 end;
                  3 : begin neux:=sx-2; neuy:=sy; t:=4 end;
                 else begin neux:=sx; neuy:=sy-2; t:=8 end;
               end;
             ...
             begin
               case sprung of
                    1 : begin neux:=sx+2; neuy:=sy;  end;
                    2 : begin neux:=sx; neuy:=sy+2;  end;
                    4 : begin neux:=sx-2; neuy:=sy;  end;
                    8 : begin neux:=sx; neuy:=sy-2;  end;
                   else exit;
                 end;
               Umsetzen;
             end;
      ...
      begin
        ...
        repeat
          ...
          if (brett[neux,neuy]=frei) and (brett[mx,my]=belegt) then
            begin
              BelegeBrett(altx,alty, neux,neuy, frei);
                ...
                then begin
                       if findAll
                         then begin
                                ...
                                inc(anzLsg);
                                labelAnzLsg.Caption:=RSArr[rsBisher]+IntToStr(anzLsg)+RSArr[rsLoesungenGefunden];
                              end
              ...
              if not erfolg
                then begin
                        BelegeBrett(altx, alty, neux, neuy, belegt);
                        if zeigen then checkPause;
                     end
            ...
            begin
              ...
              if restAnzahlPins=1
                then begin
                       ...
                       if not erfolg
                         then BelegeBrett( altx, alty, neux, neuy, belegt)
                         else begin
                                if speichern then
                                  begin
 ...
 { 1: Pin on Win-Path      - :Endposition: erhält Wert 1; alle Positionen von denen auf ein 1-er Feld gesprungen werden kann
  ...
  function solve : boolean;
    ...
    begin
     erfolg := false;
     tempAnzPinOnWinP:=0;
     tempAnzPinBetweenWinP:=0;
     tempAnzPinOutsideWinP:=0;
   ...
   if usePinType then
     begin
       ...
       case pintype[p2] of
          ...
          else dec(tempAnzPinOutsideWinP);
         end;
...
procedure TFormSolitaer.ZeigeZugFolge(aZug:TZugFolge;nr:byte);
var k, altPause  : integer;
    panelPauseVis,stoppEnabled,altShowSteps,altAbbruch  : boolean;
    BrettBackUp  : TBelegungBrett;
begin
  BrettBackUp:=Brett;   // Move(Brett,BrettBackUp,SizeOf(TBelegungBrett));
  ...
  for k:=1 to nr do  if aZug[k].vx>0 then
    begin
      with aZug[k] do
        begin
          MoveStein(vx,vy,nx,ny);
          Brett[vx,vy]  := frei;
          Brett[ (vx+nx) div 2, (vy+ny) div 2]:= frei;
          Brett[nx,ny] := belegt;
        end;
      CheckPause;
...
begin
  if not zeigen then
    begin
      if autoPlay and (bf_verfahren>2)
        then LinBrett_To_Brett;
      ZeichneBrett;
    end;
  if autoPlay and (bf_verfahren>2) then  Umrechnen_LinZug_to_ZugXY(linZug);
  if autoPlay
     then n:=zugNr
     else n:=mZugNr;
  s:='';
  for k:=1 to n do
    begin
      ...
      s1:=IntToStr(brettXYtoNr[z.vx,z.vy]);
      if Length(s1)=1 then s1:=' '+s1;
      s1:=s1+GetRichtungsChar(z)+' ';
      s:=s+s1;
    end;
  memo1.lines.add(s);
end;
